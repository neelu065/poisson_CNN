...
def handle_grid_parameters_range(value_range, ndims, output_dtype):
    ...
    if value_range.shape == 2:
        assert value_range[0] < value_range[
            1], 'Upper bound for grid spacings must be larger than or equal to the lower bound!'
        value_range = tf.expand_dims(value_range, 0)
        value_range = tf.tile(value_range, [ndims, 1])
    assert value_range.shape[
               1] == 2, 'Dim 1 of grid_spacings range must be 2, containing the lower bound and upper bound for the respective spatial dimensions'
    assert value_range.shape[0] == ndims, '1st dim of value_range must have identical size to ndims'
    assert tf.reduce_all((value_range[:, 1] - value_range[:, 0]) >= 0), 'Dims ' + str(list(
        tf.reshape(tf.where((value_range[:, 1] - value_range[:, 0]) < 0),
                   (-1,)).numpy())) + ' had upper bound of random range smaller than the lower bound'
    assert tf.reduce_all(value_range[:, 0] >= 0), 'Dims ' + str(
        list(tf.reshape(tf.where(value_range[:, 0] < 0), (-1,)).numpy())) + ' had lower bounds of random range below 0'
...
def generate_polynomial_and_second_derivative(roots, degree, npts, domain_size):
    ...
    coords = tf.linspace(tf.constant(0.0, dtype=tf.keras.backend.floatx()) - dx, 1.0 + dx,
                         npts + 2)  # extra 2 points needed for numerical stability
...
def batch_generate_polynomial_and_second_derivative(roots, degrees, npts, domain_size):
    return tf.map_fn(lambda x: generate_polynomial_and_second_derivative(x[0], x[1], npts, domain_size),
                     (roots, degrees), dtype=(tf.keras.backend.floatx(), tf.keras.backend.floatx()))
...
def polynomials_and_their_2nd_derivatives(npts, poly_deg, domain_sizes, batch_size=1, homogeneous_bc=False):
    ...
    if homogeneous_bc:
        ...
        roots = tf.concat([tf.zeros((batch_size, tf.shape(roots)[1], 1), dtype=tf.keras.backend.floatx()),
                           -tf.ones((batch_size, tf.shape(roots)[1], 1), dtype=tf.keras.backend.floatx()), roots], -1)
    else:
    ...
    p, ddp = tf.map_fn(lambda x: batch_generate_polynomial_and_second_derivative(x[0], x[1], x[2], x[3]),
                       (roots, degrees, npts, domain_sizes),
                       dtype=(tf.keras.backend.floatx(), tf.keras.backend.floatx()))
    return p, ddp


class reverse_poisson_dataset_generator(tf.keras.utils.Sequence):
    def __init__(self, batch_size, batches_per_epoch, random_output_shape_range, fourier_coeff_grid_size_range,
                 taylor_degree_range, grid_spacings_range=None, ndims=None, homogeneous_bc=False, return_rhses=True,
                 return_boundaries=True, return_dx=True, normalizations=None, uniform_grid_spacing=False):
        '''
        ...
        self.fourier_coeff_grid_size_range = handle_grid_parameters_range(fourier_coeff_grid_size_range, ndims,
                                                                          tf.int32)
        ...
        if self.return_boundaries:
            ...
            for k in range(self.ndims):
                sl0 = [Ellipsis] + [slice(0, None) for k in range(k)] + [0] + [slice(0, None) for k in
                                                                               range(ndims - k - 1)]
                sl1 = [Ellipsis] + [slice(0, None) for k in range(k)] + [-1] + [slice(0, None) for k in
                                                                                range(ndims - k - 1)]
    ...
    def generate_grid_sizes_and_spacings_with_uniform_AR(self):
        aspect_ratios = generate_uniformly_distributed_aspect_ratios(output_shape_range=self.random_output_shape_range,
                                                                     dx_range=None if self.uniform_grid_spacing else self.grid_spacings_range,
                                                                     samples=self.batch_size)
        output_shape, grid_spacings = generate_output_shapes_and_grid_spacings_from_aspect_ratios(aspect_ratios,
                                                                                                  self.random_output_shape_range,
                                                                                                  self.grid_spacings_range,
                                                                                                  constant_dx=self.uniform_grid_spacing,
                                                                                                  samples=self.batch_size)
    ...
    def generate_soln_fourier(self):
        ...
        solns, coeffs = tf.map_fn(
            lambda x: generate_smooth_function(self.ndims, x[0], x[1], homogeneous_bc=self.homogeneous_bc,
                                               normalize=False, return_coefficients=True,
                                               coefficients_return_shape=tf.reduce_max(n_coefficients, 0)),
            (output_shape, n_coefficients), dtype=(tf.keras.backend.floatx(), tf.keras.backend.floatx()))
    ...
    def generate_rhses_fourier(self, coefficients, grid_size, grid_spacings):
        ...
        wavenumbers = [tf.cast(tf.range(1, coefficients_size[k] + 1), tf.keras.backend.floatx()) for k in
                       range(self.ndims)]  # generate (n+1)*pi values/wavenumbers for each coefficient
        coefficient_adjustment = tf.stack(tf.meshgrid(*wavenumbers, indexing='ij'), 0) * tf.constant(math.pi,
                                                                                                     tf.keras.backend.floatx())  # adjust each coefficient by factor of [(n0+1)^2*pi^2/L0^2 + (n1+1)^2*pi^2/L1^2 + ...]
        ...
        rhs = tf.map_fn(lambda x: generate_smooth_function(self.ndims, x[0], x[1], homogeneous_bc=self.homogeneous_bc,
                                                           normalize=False, return_coefficients=False),
                        (grid_size, coefficients), dtype=tf.keras.backend.floatx())
    ...
    def generate_soln_and_rhs_taylor(self, grid_size, domain_sizes):
        ...
        for k in range(self.ndims):
            coeffs = 2 * tf.random.uniform([self.batch_size, polynomial_degrees[k] - 1],
                                           dtype=tf.keras.backend.floatx()) - 1
            p, ddp = polynomials_and_their_2nd_derivatives(grid_size[k], polynomial_degrees[k], domain_sizes[:, k],
                                                           batch_size=self.batch_size,
                                                           homogeneous_bc=self.homogeneous_bc)
        ...
        rhs = tf.map_fn(lambda x: self.build_taylor_rhs_component(vals=second_derivatives, indices=x), I,
    ...
    def rhs_max_magnitude_normalization(self, rhses, solns):
        rhses, scaling_factors = set_max_magnitude_in_batch_and_return_scaling_factors(rhses, self.normalizations[
            'rhs_max_magnitude'])
    ...
    def set_taylor_result_peak_magnitude_to_fourier_peak_magnitude(self, rhses_fourier, rhses_taylor, solns_taylor):
        rhses_taylor_max = tf.map_fn(lambda x: tf.reduce_max(tf.abs(x)),
                                     rhses_taylor)  # scale taylor series component to that the max magnitude is identical to the fourier series component
        ...
        taylor_scaling_coeffs = tf.reshape(taylor_scaling_coeffs,
                                           [self.batch_size] + [1 for _ in range(self.ndims + 1)])
    ...
    def __getitem__(self, idx=0):
        ...
        rhses_fourier, domain_sizes = self.generate_rhses_fourier(soln_coeffs_fourier, tf.shape(solns_fourier)[2:],
                                                                  grid_spacings_fourier)

        # taylor series component
        rhses_taylor, solns_taylor = self.generate_soln_and_rhs_taylor(tf.shape(solns_fourier)[2:], domain_sizes)
        rhses_taylor, solns_taylor = self.set_taylor_result_peak_magnitude_to_fourier_peak_magnitude(rhses_fourier,
                                                                                                     rhses_taylor,
                                                                                                     solns_taylor)
...
if __name__ == '__main__':
    ...
    rpdg = reverse_poisson_dataset_generator(batch_size=5, batches_per_epoch=5,
                                             random_output_shape_range=grid_size_range,
                                             fourier_coeff_grid_size_range=ctrl_pt_range,
                                             taylor_degree_range=ctrl_pt_range, grid_spacings_range=dxrange, ndims=2,
                                             homogeneous_bc=True, return_rhses=True, return_boundaries=False,
                                             return_dx=True, normalizations=normalizations,
