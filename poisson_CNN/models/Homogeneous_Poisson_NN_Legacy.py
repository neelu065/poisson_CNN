...
class Homogeneous_Poisson_NN_Legacy(tf.keras.models.Model):
    def __init__(self, data_format='channels_first', final_convolutions_config=None,
                 pre_bottleneck_convolutions_config=None, bottleneck_deconv_config=None,
                 bottleneck_multilinear_config=None, input_normalization=None, output_scaling=None, use_batchnorm=False,
                 postsmoother_iterations=5, use_scaling=False, use_positional_embeddings=True, scaling_config=None,
                 gradient_accumulation_steps=None, bc_type='dirichlet'):
        ...
        self._input_normalization_has_to_be_performed = tf.reduce_any(
            [bool(x) for x in self.input_normalization.values()])
        ...
        if self.output_scaling['match_peak_laplacian_magnitude_to_peak_rhs']:
            self.fd_stencil = tf.constant(poisson_CNN.dataset.utils.build_fd_coefficients(5, 2, ndims),
        ...
        pre_bottleneck_convolutions_constant_padding_value = pre_bottleneck_convolutions_config.pop(
            'constant_padding_value', 0.0)
        for k in range(len(pre_bottleneck_convolutions_config['filters'])):  #
            conv_args = get_init_arguments_from_config(pre_bottleneck_convolutions_config, k, fields_in_conv_cfg,
                                                       fields_in_conv_args)
            conv = conv_layer(data_format=data_format, padding='valid', **conv_args)
            self.pre_bottleneck_convolutions.append(conv)
            self.pre_bottleneck_convolution_ops.append(
                apply_advanced_padding_and_call_conv_layer(pre_bottleneck_convolutions_padding_mode,
                                                           self.pre_bottleneck_convolutions[-1],
                                                           constant_padding_value=pre_bottleneck_convolutions_constant_padding_value))
        ...
        fields_in_bottleneck_cfg = ['downsampling_factors', 'upsampling_factors', 'conv_kernel_sizes',
                                    'deconv_kernel_sizes', 'n_convs'] + (['conv_downsampling_kernel_sizes'] if (
                    'conv_downsampling_kernel_sizes' in bottleneck_deconv_config.keys()) else [])
        fields_in_bottleneck_args = ['downsampling_factor', 'upsampling_factor', 'conv_kernel_size',
                                     'deconv_kernel_size', 'n_convs'] + (['conv_downsampling_kernel_size'] if (
                    'conv_downsampling_kernel_sizes' in bottleneck_deconv_config.keys()) else [])
        self.bottleneck_deconv_blocks = [
            bottleneck_block_deconvupsample(ndims=ndims, data_format=data_format, use_batchnorm=self.use_batchnorm,
                                            **get_init_arguments_from_config(bottleneck_deconv_config, k,
                                                                             fields_in_bottleneck_cfg,
                                                                             fields_in_bottleneck_args)) for k in
            range(len(bottleneck_deconv_config['downsampling_factors']))]
        self.bottleneck_deconv_blocks = sorted(self.bottleneck_deconv_blocks, key=lambda x: x.downsampling_factor,
                                               reverse=True)

        fields_in_bottleneck_cfg = ['downsampling_factors', 'upsampling_factors', 'conv_kernel_sizes', 'n_convs'] + (
            ['conv_downsampling_kernel_sizes'] if (
                        'conv_downsampling_kernel_sizes' in bottleneck_multilinear_config.keys()) else []) + (
                                       ['resize_methods'] if (
                                                   'resize_methods' in bottleneck_multilinear_config.keys()) else [])
        fields_in_bottleneck_args = ['downsampling_factor', 'upsampling_factor', 'conv_kernel_size', 'n_convs'] + (
            ['conv_downsampling_kernel_size'] if (
                        'conv_downsampling_kernel_sizes' in bottleneck_multilinear_config.keys()) else []) + (
                                        ['resize_method'] if (
                                                    'resize_methods' in bottleneck_multilinear_config.keys()) else [])
        self.bottleneck_multilinear_blocks = [
            bottleneck_block_multilinearupsample(ndims=ndims, data_format=data_format, use_batchnorm=self.use_batchnorm,
                                                 **get_init_arguments_from_config(bottleneck_multilinear_config, k,
                                                                                  fields_in_bottleneck_cfg,
                                                                                  fields_in_bottleneck_args)) for k in
            range(len(bottleneck_multilinear_config['downsampling_factors']))]
        self.bottleneck_multilinear_blocks = sorted(self.bottleneck_multilinear_blocks,
                                                    key=lambda x: x.downsampling_factor, reverse=True)

        self.non_bottleneck_conv = tf.keras.layers.Conv2D(filters=bottleneck_deconv_config['filters'], kernel_size=5,
                                                          activation=tf.nn.leaky_relu, padding='same',
                                                          data_format=self.data_format)

        # merge bottleneck blocks
        # self.merge = MergeWithAttention(data_format = self.data_format)
        self.post_merge_conv = tf.keras.layers.Conv2D(filters=bottleneck_deconv_config['filters'], kernel_size=7,
                                                      activation=tf.nn.leaky_relu, padding='same',
                                                      data_format=self.data_format)
        self.post_merge_resnet = resnet(2, filters=bottleneck_deconv_config['filters'], kernel_size=7,
                                        activation=tf.nn.leaky_relu, data_format=self.data_format)
        ...
        for k in range(final_convolution_stages - self.final_regular_conv_stages):
            conv_args = get_init_arguments_from_config(final_convolutions_config, k, fields_in_conv_cfg,
                                                       fields_in_conv_args)
            conv_to_adjust_channel_number = conv_layer(data_format=data_format, padding='valid', **conv_args)
            conv = resnet(ndims=2, use_batchnorm=False, padding_mode='constant', data_format=self.data_format,
                          **conv_args)
            self.final_convolutions.append(conv_to_adjust_channel_number)
            self.final_convolution_ops.append(
                apply_advanced_padding_and_call_conv_layer(final_convolutions_padding_mode, self.final_convolutions[-1],
                                                           constant_padding_value=final_convolutions_constant_padding_value))
        ...
        for k in range(final_convolution_stages - self.final_regular_conv_stages, final_convolution_stages):
            self.final_convolutions.append(conv_layer(filters=final_convolutions_config['filters'][k],
                                                      kernel_size=final_convolutions_config['kernel_sizes'][k],
                                                      activation=tf.keras.activations.linear, padding='same',
                                                      data_format=self.data_format,
                                                      use_bias=final_convolutions_config['use_bias']))
            self.final_convolution_ops.append(self.final_convolutions[-1])
        ...
        self.dx_dense_layers = [tf.keras.layers.Dense(dx_dense_layer_units[k], activation=dx_dense_layer_activations[k])
                                for k in range(len(dx_dense_layer_units))]
        ...
        self.bc_slice = [Ellipsis] + [slice(1, -1) for _ in range(self.ndims)] + [
            slice(0, None)] if self.data_format == 'channels_last' else [Ellipsis, slice(0, None)] + [slice(1, -1) for _
                                                                                                      in
                                                                                                      range(self.ndims)]
        self.bc_paddings = [[0, 0], [0, 0]] + [[1, 1] for _ in
                                               range(self.ndims)] if self.data_format == 'channels_first' else [[0,
                                                                                                                 0]] + [
                                                                                                                   [1,
                                                                                                                    1]
                                                                                                                   for _
        ...
        self.scaling = Scaling(ndims=self.ndims, data_format=self.data_format,
                               **scaling_config) if use_scaling else None
    ...
    def match_peak_laplacian_magnitude_to_peak_rhs_scaling(self, rhs, soln, dx):
        ...
        rhs_computed = tf.map_fn(
            lambda x: self.fd_conv_method(tf.expand_dims(x[0], 0), x[1], data_format=self.data_format), (soln, kernels),
            dtype=soln.dtype)[:, 0, ...]
    ...
    def generate_position_embeddings(self, batch_size, domain_shape):
        ...
        pos_embeddings = tf.stack([tf.broadcast_to(
            tf.reshape(tf.cos(pi * tf.linspace(linspace_start, linspace_end, domain_shape[k])),
                       [1 for _ in range(k)] + [-1] + [1 for _ in range(self.ndims - k - 1)]), domain_shape) for k in
                                   range(self.ndims)], 0 if self.data_format == 'channels_first' else -1)
    ...
    def call(self, inp):
        ...
        bottleneck_results = sum(bottleneck_results) / tf.cast(
            len(bottleneck_results) * tf.shape(bottleneck_results[0])[
                1 if self.data_format == 'channels_first' else -1], tf.keras.backend.floatx())

        bottleneck_results = self.post_merge_resnet(self.post_merge_conv(
            tf.concat([self.non_bottleneck_conv(initial_conv_result), bottleneck_results],
...
if __name__ == '__main__':
    ...
    mod = Homogeneous_Poisson_NN_Legacy(final_convolutions_config=fcc, pre_bottleneck_convolutions_config=pbcc,
                                        bottleneck_deconv_config=bcc_d, bottleneck_multilinear_config=bcc_ml)
