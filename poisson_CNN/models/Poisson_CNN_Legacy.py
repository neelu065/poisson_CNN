...
from ..dataset.utils import set_max_magnitude_in_batch_and_return_scaling_factors, compute_domain_sizes, \
    flip_and_rotate_tensor
...
class Poisson_CNN_Legacy(tf.keras.models.Model):
    def __init__(self, hpnn, dbcnn, jacobi_iterations=0):
        ...
        if jacobi_iterations > 0:
            self.jacobi_iteration_layer = poisson_CNN.layers.JacobiIterationLayer([3, 3], [2, 2], 2,
                                                                                  data_format=self.hpnn.data_format,
                                                                                  n_iterations=jacobi_iterations)
        else:
    ...
    def call(self, inp):
        ...
        hpnn_result = tf.einsum('b...,b->b...', hpnn_result,
                                tf.reduce_max(compute_domain_sizes(tf.concat([inp[-1], inp[-1]], 1), tf.shape(rhs)[2:]),
                                              1) ** 2 / rhs_scaling_factors)
        ...
        top_bc_result = flip_and_rotate_tensor(top_bc_result, rotation_count=3, data_format=self.dbcnn.data_format,
                                               flip_axes=[])

        right_bc_result = self.dbcnn(
            [right, dx, tf.shape(rhs)[2]])  # /right_boundary_scaling_factors#channels_first_flip_left_right()
        ...
        bottom_bc_result = self.dbcnn(
            [bottom, dx, tf.shape(rhs)[3]])  # /bottom_boundary_scaling_factors#channels_first_rot90(, k = 3)
