...
class Dirichlet_BC_NN_Legacy_2(tf.keras.models.Model):
    def __init__(self, data_format='channels_first', boundary_conv_config=None, spp_config=None,
                 domain_info_mlp_config=None, final_convolutions_config=None, postsmoother_iterations=0,
                 use_batchnorm=False):
        ...
        if boundary_conv_config['filters'][-1] > 27 and (tf.keras.backend.floatx() == 'float32'):
            warnings.warn(str(boundary_conv_config['filters'][
                                  -1]) + ' sinh modes chosen may lead to NaN values with float32 precision. Consider using fewer than 28 when using float32.')
        ...
        for k in range(len(boundary_conv_config['filters'])):
            ...
            self.boundary_convolution_ops.append(
                apply_advanced_padding_and_call_conv_layer(boundary_conv_padding_mode, self.boundary_convolutions[-1],
                                                           constant_padding_value=boundary_conv_constant_padding_value))
            ...
            resnet_block = resnet(self.ndims - 1, use_batchnorm=self.use_batchnorm,
                                  padding_mode=boundary_conv_padding_mode,
                                  constant_padding_value=boundary_conv_constant_padding_value,
                                  data_format=self.data_format, **conv_args)
        ...
        self.spp = SpatialPyramidPool(ndims=self.ndims - 1, data_format=self.data_format, receive_padded_values=False,
                                      **spp_config)
        ...
        for k in range(len(domain_info_mlp_config['units'])):
            dense_args = get_init_arguments_from_config(domain_info_mlp_config, k, fields_in_dense_cfg,
                                                        fields_in_dense_args)
        ...
        for k in range(final_convolution_stages - self.final_regular_conv_stages):
            ...
            conv = resnet(ndims=self.ndims, use_batchnorm=False, padding_mode='constant', data_format=self.data_format,
                          **conv_args)
        ...
        for k in range(final_convolution_stages - self.final_regular_conv_stages, final_convolution_stages):
            self.final_convolutions.append(domain_conv_layer(filters=final_convolutions_config['filters'][k],
    ...
    def build_series_x_dir_components(self, x_output_resolution):
        xbar = tf.linspace(tf.constant(0.0, tf.keras.backend.floatx()), tf.constant(1.0, tf.keras.backend.floatx()),
                           x_output_resolution)
    ...
    def call(self, inp):
        ...
        bc_conv_result = self.boundary_convolution_ops[0](
            tf.concat([bc, pos_embeddings_nminus1d], 1 if self.data_format == 'channels_first' else -1))
        ...
        out = tf.einsum(self._einsum_boundary_conv_input_str + ',mx,bm->' + self._einsum_output_str, bc_conv_result,
                        sinh_values, dense_result)
        ...
        out = tf.concat([tf.expand_dims(bc, self.direction_perpendicular_to_bc), out[self.concat_bc_output_slice]],
                        self.direction_perpendicular_to_bc)
    ...
    def train_step(self, data):
        ...
        with tf.GradientTape() as tape:
            ...
            loss = self.loss_fn(y_true=ground_truth, y_pred=pred,
                                rhs=tf.zeros(tf.shape(ground_truth), dtype=ground_truth.dtype),
                                dx=tf.concat([dx, dx], 1))
...
if __name__ == '__main__':
    ...
    mod = Dirichlet_BC_NN_Legacy_2(data_format='channels_first', boundary_conv_config=bccfg, spp_config=sppcfg,
                                   domain_info_mlp_config=mlpcfg, final_convolutions_config=fccfg,
                                   postsmoother_iterations=0, use_batchnorm=True)
