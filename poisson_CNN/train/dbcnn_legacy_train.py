...
from poisson_CNN.models import Dirichlet_BC_NN_Legacy_2
from poisson_CNN.losses import loss_wrapper
from poisson_CNN.dataset.generators import numerical_dataset_generator
...
cb = [
    ...
    hvd.callbacks.LearningRateWarmupCallback(initial_lr=config['training']['optimizer_parameters']['learning_rate'], warmup_epochs=3, verbose=verbose),
...
if hvd.rank() == 0:
    cb.append(tf.keras.callbacks.ModelCheckpoint(checkpoint_dir + '/chkpt-epoch-{epoch:02d}.mse-{mse:.4f}',save_weights_only=True,save_best_only=True,monitor = 'loss',verbose=verbose))
...
model.summary()
model.fit(dataset,epochs=config['training']['n_epochs'],callbacks=cb)         #, initial_epoch=93)
