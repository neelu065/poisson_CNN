...
def get_inputs():
    ...
    try:
        ...
        time_input = input(
            'Time must be float or integers, separate start and end time by a comma (,) now try again: ').split(',')
    ...
    if test_problem_name == 'driven_cavity':
        ...
        if 'Y' in Error_analysis_option or 'y' in Error_analysis_option:
            ...
            Niter = input(
                'How many times you want to run the solver? (Default is 5 and this corresponds to grid size of 15, 30, 60, 120 and 240): ')
            while Niter == '':
                # take default value
                Niter = 5
            try:
...
def error_analysis(xl, xr, t0, tf, method, test_problem_name, CFL=0.1, Re=1.0, Niter=5):
    ...
    for i in gridsizel:
        gridsize = i
        Velocity_error, Pressure_error, avg_gradp_error, dt = run_Navier_Stokes_solver(xl, xr, t0, tf, gridsize, method,
                                                                                       test_problem_name,
                                                                                       plot_option=False, CFL=CFL)
    ...
    P_slope_Linf, P_intercept_Linf, r_value, p_value, std_err = stats.linregress(np.array(log_dt)[1:],
                                                                                 log_PLinf_error[1:])
    ...
    plt.figure(1)
    plt.plot(log_dt, log_dt * U_slope_L1 + U_intercept_L1,
             label='L1 velocity error, convergence rate = %s' % U_slope_L1, color='r')
    plt.scatter(log_dt, log_UL1_error, color='r')
    plt.plot(log_dt, log_dt * U_slope_L2 + U_intercept_L2,
             label='L2 velocity error, convergence rate = %s' % U_slope_L2, color='b')
    plt.scatter(log_dt, log_UL2_error, color='b')
    plt.plot(log_dt, log_dt * U_slope_Linf + U_intercept_Linf,
             label='Linf velocity error, convergence rate = %s' % U_slope_Linf, color='g')
    plt.scatter(log_dt, log_ULinf_error, color='g')
    plt.plot(log_dt, log_dt * 2 + min([U_intercept_L1, U_intercept_L2, U_intercept_Linf]) - 1, '--',
             label='2nd order reference line', color='k', linewidth=1.5)
    plt.xlabel('log(dt)')
    plt.ylabel('log(error)')
    plt.title(
        'log log plot of the temporal error convergence rates for velocity, method: ' + method + ', flow problem: ' + test_problem_name + ', CFL = %s' % CFL)
    plt.legend(loc=4)

    # for pressure
    plt.figure(2)
    plt.plot(log_dt, log_dt * P_slope_L1 + P_intercept_L1 + 3,
             label='L1 pressure error, convergence rate = %s' % P_slope_L1, color='r')
    plt.scatter(log_dt, log_PL1_error + 3, color='r')
    plt.plot(log_dt, log_dt * P_slope_L2 + P_intercept_L2,
             label='L2 pressure error, convergence rate = %s' % P_slope_L2, color='b')
    plt.scatter(log_dt, log_PL2_error, color='b')
    plt.plot(log_dt, log_dt * P_slope_Linf + P_intercept_Linf - 3,
             label='Linf pressure error, convergence rate = %s' % P_slope_Linf, color='g')
    plt.scatter(log_dt, log_PLinf_error - 3, color='g')
    plt.plot(log_dt, log_dt * 2 + min([P_intercept_L1 + 3, P_intercept_L2, P_intercept_Linf - 1]) - 3, '--',
             label='2nd order reference line', color='k', linewidth=1.5)
    plt.xlabel('log(dt)')
    plt.ylabel('log(error)')
    plt.title(
        'log log plot of the temporal error convergence rates for pressure, method: ' + method + ', flow problem: ' + test_problem_name + ', CFL = %s' % CFL)
    plt.legend(loc=4)
...
def run_Navier_Stokes_solver(xl, xr, t0, tf, gridsize, method, test_problem_name, plot_option, CFL=0.1, Re=1.0):
    ...
    if test_problem_name == 'driven_cavity':
        ...
        uv_exact_bnd, p_exact, gradp_exact = structure.Exact_solutions(mesh, Re, mesh.Tn).Exact_solutions(
            test_problem_name)
    ...
    if plot_option == False:
        ...
        if test_problem_name == 'driven_cavity':
            ...
            ax7.plot_surface(XPint, YPint, pf.get_value() - p_exact.get_value(), rstride=1, cstride=1, cmap=cm.jet,
                             linewidth=0.2)
            ax7.set_xlabel('x')
            ax7.set_ylabel('y')
...
if __name__ == "__main__":
    print(
        'WARNING: At the moment, this simulation code uses hard coded file paths in the solvers.py file and the usage of the model is commented out (line 250 thereabouts). Change these to use this script properly!')
    inputs = get_inputs()
