...
class mesh:
    ...
    def __init__(self, gridsize, spatial_domain, time_domain, CFL, Re):
        ...
        self.yu = np.linspace(start=self.sdomain[1][0] + 0.5 * self.dy, stop=self.sdomain[1][1] - 0.5 * self.dy,
                              num=self.m)
        self.xv = np.linspace(start=self.sdomain[0][0] + 0.5 * self.dx, stop=self.sdomain[0][1] - 0.5 * self.dx,
                              num=self.n)
...
class Exact_solutions():
    ...
    def Exact_solutions(self, Solution_type):
        ...
        return VelocityField(U_exact_bnd, V_exact_bnd, self.mesh), CentredPotential(P_exact, self.mesh), VelocityField(
            gradpu_exact, gradpv_exact, self.mesh)
...
class Forcing_term:
    ...
    def periodic_forcing_1(self):
        ...
        Fx = np.pi * np.cos(tn) * np.sin(2 * np.pi * Yu) * (np.sin(np.pi * Xu) ** 2) - \
             2 * (np.pi ** 3) * np.sin(tn) * np.sin(2 * np.pi * Yu) * (
                         np.cos(2 * np.pi * Xu) - 2 * (np.sin(np.pi * Xu) ** 2)) - \
             np.pi * np.sin(tn) * np.sin(np.pi * Yu) * np.sin(np.pi * Xu)

        Fy = -np.pi * np.cos(tn) * np.sin(2 * np.pi * Xv) * (np.sin(np.pi * Yv) ** 2) - \
             2 * (np.pi ** 3) * np.sin(tn) * np.sin(2 * np.pi * Xv) * (
                         2 * (np.sin(np.pi * Yv) ** 2) - np.cos(2 * np.pi * Yv)) + \
